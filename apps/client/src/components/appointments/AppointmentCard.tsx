      ...
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-3">
            {/* Avatar */}
            <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-teal-600 rounded-full flex items-center justify-center text-white font-bold shadow-lg">
              {getInitials(personName)}
            </div>
            
            {/* Info */}
            <div>
              <p className="font-bold text-gray-900">{personName}</p>
              {userRole === 'PATIENT' && appointment.doctor?.doctorProfile?.specialization && (
                <p className="text-sm text-gray-500">{appointment.doctor.doctorProfile.specialization}</p>
              )}
        ...
        <div>
          {appointment.status === 'SCHEDULED' ? (
            ...
            <button 
              ...
              className="bg-white hover:bg-gray-50 text-gray-700 border-2 border-gray-300 px-6 py-3 rounded-xl font-semibold transition hover:shadow-md flex items-center gap-2"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
